...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            puzzleRangeProofs[i][j].in <== puzzle[i][j]; //[assignment] change assert() to use your created RangeProof instead
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            puzzleRangeProofs[i][j].out === 1;
            solutionRangeProofs[i][j].out === 1;
